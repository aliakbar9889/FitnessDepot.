...
export default function PromoProductSlider2() {
  ...
  useEffect(() => {
    const getProducts = async () => {
      try {
        ...
        const commercialProducts = (data.products || []).filter(
          (product: Product) => product.isCommercial === true
        );
  ...
  const displayedProducts = products.slice(0, eightyPercentCount);

  if (loading) {
    return (
      <section className="flex min-h-[320px] w-full items-center justify-center bg-white px-4 sm:min-h-[420px]">
  ...
  return (
    <>
      <section className="w-full max-w-full overflow-hidden bg-white px-3 py-8 sm:px-5 sm:py-10 md:px-8 lg:px-12 lg:py-12">
        <div className="mx-auto grid w-full max-w-7xl min-w-0 grid-cols-1 gap-5 sm:gap-6 lg:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[300px_minmax(0,1fr)]">
          ...
          <div className="relative h-[340px] w-full overflow-hidden rounded-2xl sm:h-[380px] lg:h-full lg:min-h-[380px]">
            <Image
              src="/products/pic2.avif"
              alt="Premium Gym Commercial Equipment"
              fill
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-5 text-white sm:p-6">
              ...
              <p className="mt-3 max-w-[330px] text-xs leading-5 text-white/80 sm:text-sm sm:leading-6">
                Discover premium gym commercial equipments designed
          ...
          <div className="relative min-w-0 max-w-full overflow-hidden">
            <div className="mb-5 flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex w-full items-center gap-2 sm:w-auto">
                <Link
                  href="/commercial"
                  className="flex h-10 flex-1 items-center justify-center rounded-full border border-gray-200 px-3 text-xs font-medium transition hover:bg-black hover:text-white sm:w-32 sm:flex-none sm:px-4 sm:text-sm md:w-36"
                >
                  View All
                </Link>
            ...
            {displayedProducts.length > 0 ? (
              <div
                ref={sliderRef}
                className="flex w-full max-w-full gap-3 overflow-x-auto overflow-y-hidden scroll-smooth pb-4 sm:gap-4"
                style={{
                  scrollbarWidth: "none",
              ...
              >
                {displayedProducts.map((product) => (
                  <article
                    key={product._id}
                    className="group w-[175px] min-w-[175px] shrink-0 overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg sm:w-[210px] sm:min-w-[210px] md:w-[220px] md:min-w-[220px]"
                  >
                    <div className="relative h-[155px] w-full overflow-hidden bg-white sm:h-[180px] md:h-[190px]">
                      <Image
                        ...
                        alt={product.name}
                        fill
                        sizes="(max-width: 639px) 175px, (max-width: 767px) 210px, 220px"
              ...
              <div className="flex min-h-[220px] w-full items-center justify-center rounded-xl bg-gray-50 px-4 sm:min-h-[300px]">
